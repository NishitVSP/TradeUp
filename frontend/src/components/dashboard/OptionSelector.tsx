...
export function OptionSelector() {
  ...
  const getStrikePrice = () => atmStrike ? atmStrike + selectedStrikeOffset * (STRIKE_STEP[selectedIndex] ?? 50) : null;
  ...
  const handleAddContract = async () => {
    ...
    if (!strikePrice || availableExpiries.length === 0) { dispatch(setError('Please wait for market data to load')); return; }
    ...
    try {
      ...
      if (data.success) {
        dispatch(addWatchedContract({
          ...
          updateInterval: Math.abs(selectedStrikeOffset) <= 5 ? 500 : Math.abs(selectedStrikeOffset) <= 10 ? 1000 : 2000,
        }));
        dispatch(addToTerminal({
          contract: { indexName: selectedIndex, strikePrice, expiryDate, optionType, ltp: data.data?.ltp ?? null },
          expiryDate,
          atmStrike: atmStrike!,
    ...
    } catch { dispatch(setError('Network error. Please try again.')); }
        ...
        <FormControl fullWidth size="small">
          ...
          <Select value={selectedStrikeOffset} label="Strike" onChange={(e) => dispatch(setSelectedStrikeOffset(e.target.value as number))} sx={{ fontFamily: '"DM Sans", sans-serif' }}>
            {Array.from({ length: 81 }, (_, i) => i - 40).map((offset) => (
              <MenuItem key={offset} value={offset}>{offset === 0 ? 'ATM' : offset > 0 ? `ATM+${offset}` : `ATM${offset}`}</MenuItem>
            ))}
          </Select>
        </FormControl>
        <Box sx={{ p: 1.5, background: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
          <Box sx={{ fontSize: '0.75rem', color: '#64748b', fontFamily: '"DM Sans", sans-serif', mb: 0.5 }}>Selected Strike</Box>
          <Box sx={{ fontSize: '1.1rem', fontWeight: 700, color: '#1e293b', fontFamily: '"DM Sans", sans-serif' }}>{getStrikeLabel()}</Box>
        </Box>
        <FormControl fullWidth size="small">
          <InputLabel>Expiry</InputLabel>
          <Select value={selectedExpiryIndex} label="Expiry" onChange={(e) => dispatch(setSelectedExpiryIndex(e.target.value as number))} sx={{ fontFamily: '"DM Sans", sans-serif' }} disabled={availableExpiries.length === 0}>
            {availableExpiries.map((expiry, i) => <MenuItem key={expiry} value={i + 1}>{expiry}{i === 0 ? ' (Nearest)' : ''}</MenuItem>)}
          </Select>
        </FormControl>
        <FormControl fullWidth size="small">
